...
from .globals import (
    ...
    DECOMPRESSION_JOB_PREFIX, FASTQ_SET_PREFIX
)
...
async def sanitise_fqs_orcabus_id(fastq_id: str) -> str:
    if ORCABUS_ULID_REGEX_MATCH.match(fastq_id):
        return fastq_id
    elif ORCABUS_ULID_REGEX_MATCH.match(f"{FASTQ_SET_PREFIX}.{fastq_id}"):
        return f"{FASTQ_SET_PREFIX}.{fastq_id}"
    raise ValueError(f"Invalid fastq list row id '{fastq_id}'")


async def sanitise_fdj_orcabus_id(job_id: str) -> str:
    if ORCABUS_ULID_REGEX_MATCH.match(job_id):
        return job_id
    elif ORCABUS_ULID_REGEX_MATCH.match(f"{DECOMPRESSION_JOB_PREFIX}.{job_id}"):
        return f"{DECOMPRESSION_JOB_PREFIX}.{job_id}"
...
def get_decompression_endpoint_url() -> str:
    return environ.get("DECOMPRESSION_BASE_URL") + "/api/v1/jobs/"
